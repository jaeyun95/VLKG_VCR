...
from models.gcn.pygcn.models import GCN
...
class LSTMBatchNormBUANonTagGlobalFullNoFinalImage(Model):
    ...
    def _collect_obj_reps(self, span_tags, object_reps):
        ...
        retrieved_featus = torch.zeros(span_tags.shape[0],span_tags.shape[1],span_tags.shape[2],512).cuda()
        #print('retrieved : ',retrieved_featus.shape)
        for i in range(4):
            example = object_reps[:,i,:,:]
            example2 = example[row_id.view(-1), span_tags_fixed.view(-1)].view(*span_tags_fixed.shape, -1)
            retrieved_featus[:,i,:,:] = example2[:,i,:,:]
        #print(retrieved_featus.shape)
        #return object_reps[row_id.view(-1), span_tags_fixed.view(-1)].view(*span_tags_fixed.shape, -1)
        return retrieved_featus
    ...
    def node_compress(self, node, visual_concept, question):
        ...
        padd_pre_node_rep = padd_pre_node_rep.view(padd_pre_node_rep.shape[0], padd_pre_node_rep.shape[1],
                                                   padd_pre_node_rep.shape[2] * padd_pre_node_rep.shape[3])
        padd_pre_node_rep = torch.unsqueeze(padd_pre_node_rep, 2)
        padd_pre_node_rep = padd_pre_node_rep.expand(padd_pre_node_rep.shape[0], padd_pre_node_rep.shape[1],
                                                     node['bert'].shape[2], padd_pre_node_rep.shape[3])
    ...
    def forward(self,
        ...
        #print('gggggggggggggggggggggg ',gcn_rep.shape)
        # image part
        ...
        final_material_rep = torch.cat((query_option_image_cat, final_node_rep_pool), -1)
        #print('final~~~~~~~~',final_material_rep.shape)

        #logits = self.final_mlp_linear(query_option_image_cat)
        logits = self.final_mlp_linear(final_material_rep).squeeze(2)
