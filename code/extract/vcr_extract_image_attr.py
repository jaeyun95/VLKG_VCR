...
################################################
#output_h5 = h5py.File(f'extract_feature/vcr_attribute_image_val.h5', 'w')
#output_h5_2 = h5py.File(f'extract_feature/vcr_attribute_image_train.h5', 'w')
################################################
#start now!
################################################
check_list = []
...
for i,item in enumerate(val_loader):
    ...
    group2use.create_dataset(f'features', data=tag_features)
    group2use.create_dataset(f'boxes', data=tag_boxes)
    group2use.create_dataset(f'num_boxes', data=tag_boxes.shape[0])
    '''
check_list = []
print('train start!')
for i, item in enumerate(train_loader):
        ...
        # print('boxes ', boxes)
        num_boxes = boxes.shape[0]
        if (tag_boxes.shape[0] != num_boxes):
            print("train!!!!! different!!!")
        tag_feature_path = os.path.join('/YOUR_PATH/',
