...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        self.h5fn = os.path.join(VCR_ANNOTS_DIR, 'vlversion','new_tags','annotation', f'vl_{self.embs_to_load}_{self.mode}_{self.split}.h5')
        ...
        self.tag_annot_path = os.path.join('/media/ailab/songyoungtak/vcr_new/tab_vcr/attribute_features/',
                                             f'attribute_features_{self.split}.h5')

        # load non-tag related stuff
        print('loading non tag image features')
        self.non_tag_annot_path = os.path.join('/media/ailab/songyoungtak/vcr_new/tab_vcr/new_tag_features/',
                                                 f'new_tag_features_{self.split}.h5')


        # load tag related stuff
        print ('loading tag image features')
        self.tag_feature_path = os.path.join('/media/ailab/songyoungtak/vcr_new/new/add_keyword/vlversion/new_tags/image/tags/', f'attribute_features_{self.mode}_{self.split}.h5')

        # load non-tag related stuff
        print ('loading non tag image features')
        self.non_tag_feature_path = os.path.join('/media/ailab/songyoungtak/vcr_new/new/add_keyword/vlversion/new_tags/image/non_tags/', f'new_tag_{self.mode}_{self.split}.h5')
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.non_tag_feature_path, 'r') as h5:
            non_tag_boxes = np.array(h5[str(anno_id)]['boxes'], dtype=np.float32)
            non_tag_features = np.zeros([4, non_tag_boxes.shape[0], 1024])
            for m in range(4):
                non_tag_features[m, :, :] = np.array(h5[str(anno_id)]['features' + str(m)], dtype=np.float32)
        ...
        for z in range(4):
            ...
            non_tag_det_features[z,:,:] = non_tag_features[z][non_tag_dets2use]
        det_features = np.concatenate((tag_det_features, non_tag_det_features),1)
